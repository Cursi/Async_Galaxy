...
public class SpaceExplorer extends Thread
{
	...
	public void run()
    {
        while(true)
        {
            Message currentMessage = this.channel.getMessageHeadQuarterChannel();

            if(currentMessage != null && !this.discovered.contains(currentMessage.getCurrentSolarSystem()))
            {
                currentMessage.setData(encryptMultipleTimes(currentMessage.getData(), this.hashCount));
                this.discovered.add(currentMessage.getCurrentSolarSystem());
                this.channel.putMessageSpaceExplorerChannel(currentMessage);
            }
        }
    }
